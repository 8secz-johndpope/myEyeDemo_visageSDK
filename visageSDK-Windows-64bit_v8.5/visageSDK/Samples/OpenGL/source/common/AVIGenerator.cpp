...
: m_sFile(_T("Untitled.avi")), m_dwRate(25),
...
CAVIGenerator::CAVIGenerator(LPCTSTR sFileName, DWORD dwRate)
...
#ifdef _AVIGENERATOR_USE_MFC
void CAVIGenerator::SetBitmapHeader()
{
	// initialize m_bih
	memset(&m_bih,0, sizeof(BITMAPINFOHEADER));
...
//	AVICOMPRESSOPTIONS aco ;
   ...
   /*if (!AVISaveOptions(NULL, 0, 1, &m_pStream, (LPAVICOMPRESSOPTIONS FAR *) &aopts))
	...
	opts.fccHandler = mmioFOURCC('X','V','I','D') ; // izaberi XviD codec, bez popup codec dijaloga
...
BOOL CAVIGenerator::SaveCombinedFile(LPTSTR lpszFileName, PAVISTREAM paviVideo, PAVISTREAM paviAudio)
{
	int                  i;
	PAVISTREAM           pavis[2];
	...
	if (AVISaveV(lpszFileName, NULL, NULL, 2, pavis,(LPAVICOMPRESSOPTIONS FAR *) &options) != AVIERR_OK) {
		return FALSE;
	}
...
int CAVIGenerator::JoinStreamsAndSaveFile()
{
	...
	if (AVIStreamOpenFromFile(&paviVideo, TEXT("test.av6.avi"), streamtypeVIDEO, 0, OF_READ, NULL) != 0) {
		AVIFileExit();
		return 0;
	}

	if (AVIStreamOpenFromFile(&paviAudio, TEXT("test.av9.wav"), streamtypeAUDIO, 0, OF_READ, NULL) != 0) {
		AVIStreamRelease(paviVideo);
		AVIFileExit();
		return 0;
	}
	
	if (SaveCombinedFile(TEXT("avi+wav.avi"), paviVideo, paviAudio))

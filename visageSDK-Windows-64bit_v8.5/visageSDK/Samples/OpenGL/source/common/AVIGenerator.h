...
public:
	...
	CAVIGenerator(LPCTSTR sFileName, DWORD dwRate);
#endif
	...
	*/
	...
	//! Join audio stream from audio file with video stream from video file.
	int JoinStreamsAndSaveFile();
	//! Get streams from files. Join streams and save them to a new file.
	BOOL SaveCombinedFile(LPTSTR lpszFileName, PAVISTREAM paviVideo, PAVISTREAM paviAudio);

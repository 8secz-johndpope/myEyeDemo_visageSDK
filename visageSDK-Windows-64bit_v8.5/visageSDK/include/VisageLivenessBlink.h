...
//
/////////////////////////////////////////////////////////////////////////////
#ifndef VSDK_VISAGELIVENESSBLINK_H
#define VSDK_VISAGELIVENESSBLINK_H
...
namespace VisageSDK
{
	...
	* In order for action to be detected accurately, it is required that the face is frontal for 30 frames
	* before performing blink.\n During blink verification, face data from latest 30 frames ("face data sequence"), i.e. last 30 calls of update() is used to verify head stability (STATE_WAIT_FOR_STABLE).
	...
	*	4. <b>STATE_WAIT_FOR_ACTION</b> Data is processed in order to verify blink.
	*	5. <b>STATE_ACTION_VERIFIED</b> Action is verified.
	*	.
	*
	*/
	class VISAGE_DECLSPEC VisageLivenessBlink : public VisageLivenessAction {
	...
	public:
		...
		* Returns action's current state so appropriate message and/or image can be displayed if user interaction is required (e.g. warning for user or instruction on how to perform blink).
		...
		*/
		int update(const FaceData *faceData, VsImage *frame = 0);
...
#endif //VSDK_VISAGELIVENESSBLINK_H